  ...
  >
    <div class="mx-auto flex max-w-7xl items-center h-20 px-4 lg:px-8">
        ...
        <template v-for="link in navLinks" :key="link.href">
          ...
          >
            <NuxtLink
              ...
              :class="isActive(link.href) ? '!text-orange-500 !font-semibold border-b-orange-500' : 'text-[#424444] hover:text-orange-500'"
            >
              {{ link.label }}
            ...
            <div
              ...
              class="absolute left-1/2 top-20 min-w-[220px] -translate-x-1/2"
              @mouseenter="handleMouseEnter(link.dropdown)"
              @mouseleave="handleMouseLeave"
            >
              <div class="mt-1 rounded-lg bg-white py-2 shadow-[0_8px_40px_rgba(0,0,0,0.12)] border border-gray-200/50">
                <NuxtLink
                  v-for="item in getDropdownItems(link.dropdown)"
                  :key="item.href"
                  :to="item.href"
                  class="block px-5 py-2.5 text-sm text-[#424444] transition-all duration-200 hover:bg-orange-500/[0.06] hover:text-orange-500 hover:pl-6"
                >
                  {{ item.label }}
                </NuxtLink>
              </div>
      ...
      <div class="hidden shrink-0 items-center gap-3 lg:flex">
        ...
        <div class="group text-center z-20 hover:z-[100]">
          <div class="w-12 h-12 rounded-lg overflow-hidden border border-gray-200 cursor-pointer transition-all duration-300 origin-top group-hover:scale-[2.8] group-hover:translate-y-[40px] group-hover:shadow-xl group-hover:border-orange-500">
            <NuxtImg src="/images/qr-2.png" alt="销售咨询" width="48" height="48" class="w-full h-full object-cover" />
          </div>
        </div>
        <div class="group text-center z-10 hover:z-[100]">
          <div class="w-12 h-12 rounded-lg overflow-hidden border border-gray-200 cursor-pointer transition-all duration-300 origin-top group-hover:scale-[2.8] group-hover:translate-y-[40px] group-hover:shadow-xl group-hover:border-orange-500">
            <NuxtImg src="/images/qr-china.png" alt="销售专员（中国办）" width="48" height="48" class="w-full h-full object-cover" />
          </div>
        </div>
      </div>
    ...
    <!-- Mobile menu -->
    <div v-if="mobileOpen" class="border-t border-gray-200 bg-white lg:hidden max-h-[calc(100vh-80px)] overflow-y-auto">
      <nav class="mx-auto flex max-w-7xl flex-col px-4 py-3">
        <template v-for="link in navLinks" :key="link.href">
          <NuxtLink
            ...
            :class="isActive(link.href) ? 'text-orange-500 font-semibold' : 'text-gray-900 hover:text-orange-500'"
          >
            {{ link.label }}
          </NuxtLink>
          <div v-if="link.dropdown" class="ml-3 mb-2 flex flex-col gap-0.5 border-l-2 border-gray-200 pl-3">
            <NuxtLink
              ...
              :key="item.href"
              :to="item.href"
              class="py-1.5 text-sm text-gray-500 hover:text-orange-500 transition-colors"
            >
              {{ item.label }}
            </NuxtLink>
          </div>
          ...
          <div class="flex gap-4">
            <div class="text-center">
              <div class="w-20 h-20 rounded-lg overflow-hidden border border-gray-200">
                <NuxtImg src="/images/qr-2.png" alt="销售咨询" width="80" height="80" class="w-full h-full object-cover" />
              </div>
              <p class="text-xs text-gray-500 mt-1">销售咨询</p>
            </div>
            <div class="text-center">
              <div class="w-20 h-20 rounded-lg overflow-hidden border border-gray-200">
                <NuxtImg src="/images/qr-china.png" alt="销售专员（中国办）" width="80" height="80" class="w-full h-full object-cover" />
              </div>
